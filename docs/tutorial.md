...
custom_config_base: "/dev/null" # Disable remote nf-core/configs loading
```
...
- **`custom_config_base: "/dev/null"`** — The pipeline normally tries to load institutional configs from a remote URL, which fails. We supply the Cheaha config locally via `-c` instead.
...
| `Config file does not exist: .../workflows/sra/nextflow.config` | Pipeline tries to load remote pipeline-specific config | Verify `custom_config_base: "/dev/null"` in params |
